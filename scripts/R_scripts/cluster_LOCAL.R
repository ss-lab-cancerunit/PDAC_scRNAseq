...
setdiff(all_celltypes, names(celltype_colours))

names(celltype_colours)[
  duplicated(unname(celltype_colours))
]
...
Assays(PDAC_met_filtered)

PDAC_primary <- map_symbols(PDAC_primary)
PDAC_met <- map_symbols(PDAC_met)
...
saveRDS(PDAC_primary, file = "/Users/thirisantracy/Desktop/thesis/PDAC_primary_filtered.rds")
saveRDS(PDAC_met, file = "/Users/thirisantracy/Desktop/thesis/PDAC_met_filtered.rds")


primary_meta <- PDAC_primary_filtered@meta.data %>%
  mutate(
    tumor = "Primary",
...
met_meta <- PDAC_met_filtered@meta.data %>%
  mutate(
    tumor = "Metastasis",
    patient = as.character(patient),
    celltype_annotation = as.character(celltype_annotation)
...
composition_meta <- bind_rows(primary_meta, met_meta)

sample_cell_counts <- composition_meta %>%
  dplyr::count(
    dataset_GSE,
    tumor,
    patient,
  ...
  arrange(tumor, dataset_GSE, patient)

View(sample_cell_counts)

sample_celltype_counts <- composition_meta %>%
  dplyr::count(
    dataset_GSE,
    tumor,
    patient,
...
primary_composition_plot <- ggplot(
  ...
  labs(
    title = "Cell-type composition of primary PDAC samples",
    x = "Patient sample",
...
met_composition_plot <- ggplot(
  ...
  labs(
    title = "Cell-type composition of metastatic PDAC samples",
    x = "Patient sample",
    y = "Proportion of cells",
    fill = NULL
  ) +
